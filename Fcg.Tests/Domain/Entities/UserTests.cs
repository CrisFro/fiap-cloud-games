...
namespace Fcg.Tests.UnitTests
{
    ...
    public class UserTests
    {
        ...
        public void RemoveGameFromLibrary_ExistingGame_ShouldRemoveGameAndTrackAsRemoved()
        {
            ...
            // Verifica��es iniciais para garantir o Arrange correto
            Assert.Contains(user.Library, ug => ug.Game.Id == game.Id);
            ...
            Assert.Equal(initialUserGaming.Id, user.GamesRemoved.First().Id); // Verifica que � a mesma inst�ncia de UserGaming removida
            Assert.Empty(user.GamesAdded); // Certifique-se de que nada foi "adicionado" durante a remo��o
        }
